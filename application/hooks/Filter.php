    ...
    private function run_filter(array & $filter)
    {
        ...
        if ($this->is_valid_action($filter['action'])) {
            ...
            if ($only && isset($filter['except']) && !empty($filter['except'])) {
                $this->CI->logger->error('Only and Except are both set in filter with action ' . $filter['action']);
            }
            ...
            $this->CI->logger->error($filter['action'] . ' is not a valid action.  Ignoring in filter.');
        }
    }
